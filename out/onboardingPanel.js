...
const systemDetector_1 = require("./systemDetector");
class OnboardingPanel {
    static show(context, client) {
        if (OnboardingPanel.currentPanel) {
            OnboardingPanel.currentPanel._panel.reveal();
            return;
        }
        const panel = vscode.window.createWebviewPanel('ollamaSetup', 'Ollama AI — Setup', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    constructor(panel, context, client) {
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case 'getSystemInfo': {
                    const sys = (0, systemDetector_1.detectSystem)();
                    const recs = (0, systemDetector_1.getRecommendedModels)(sys);
                ...
                case 'installModel': {
                    ...
                    vscode.window.showInformationMessage(`Downloading ${modelId}. Watch the terminal window.`, 'Go to Terminal').then(sel => { if (sel) {
                        terminal.show();
                    } });
                    break;
                }
                case 'saveModel': {
                    await vscode.workspace.getConfiguration('ollamaAI').update('model', msg.modelId, vscode.ConfigurationTarget.Global);
    ...
    _buildHtml() {
        const models = systemDetector_1.ALL_MODELS;
