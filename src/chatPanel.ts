...
import { buildMultiFileContext, buildPrompt, getActiveFileContext, getOpenEditorsContext, getSelectionContext } from './fileContext';
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  private _skill: 'code' | 'chat' | 'plan' | 'editor' = 'code';
  ...
  private async _ensureActiveSession() {
  ...
  private _getSystemMessage(): string {
    switch (this._skill) {
      case 'chat':
  ...
  private async _handleSend(userText: string, attachedPaths: string[] = []) {
    ...
    if (this._skill === 'code') {
      const explicitExtra = attachedPaths.length > 0
        ? buildMultiFileContext(this._dedupePaths(attachedPaths))
        : '';
      const openTabsExtra = !explicitExtra ? getOpenEditorsContext() : '';
      prompt = buildPrompt(
        userText,
        getActiveFileContext(this._getContextLines()),
        explicitExtra || openTabsExtra
      );
    } else {
    ...
    const apiMessages: OllamaMessage[] = systemMsg
  ...
  private async _handleSetModel(model: string | undefined) {
    if (!model) {
      return;
    }
  ...
  private async _handleSwitchSession(sessionId: string | undefined) {
    if (!sessionId) {
      return;
    }
  ...
  private async _handleDeleteSession(sessionId: string | undefined) {
    if (!sessionId) {
      return;
    }
  ...
  private async _handleApplyAssistantEdits(responseText: string | undefined) {
    ...
    const resolvedEdits = edits
      ...
      .filter((edit): edit is { relativePath: string; uri: vscode.Uri; content: string } => !!edit);
  ...
  private async _postSessionState() {
    this._post({
      command: 'sessionList',
  ...
  private async _searchWorkspaceFiles(query: string): Promise<WorkspaceFileResult[]> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
      return [];
    }
  ...
  private _scoreWorkspaceFileMatch(query: string, file: WorkspaceFileResult): number {
    const name = file.name.toLowerCase();
    const rel  = file.relativePath.toLowerCase();
    ...
    if (rel.includes(query))              { return 4; }

    // Fuzzy: all query chars appear in order in the name
    if (this._fuzzyMatch(query, name))    { return 5; }
    if (this._fuzzyMatch(query, rel))     { return 6; }

    return 7;
  }

  private _fuzzyMatch(query: string, target: string): boolean {
    let qi = 0;
  ...
  private _isContextFriendlyFile(filePath: string): boolean {
  ...
  private _extractAssistantFileEdits(responseText: string): AssistantFileEdit[] {
    const edits = new Map<string, AssistantFileEdit>();
    const regex = /```file:([^\r\n`]+)\r?\n([\s\S]*?)```/g;
  ...
  private _normalizeAssistantRelativePath(relativePath: string): string | null {
  ...
  private async _handleDeleteModel(modelId: string | undefined) {
    if (!modelId) { return; }
    const choice = await vscode.window.showWarningMessage(
      `Delete model "${modelId}"? This will remove it from Ollama.`,
  ...
  private _handlePullModel(modelId: string | undefined) {
  ...
  private _getTemperature(): number {
    return vscode.workspace.getConfiguration('ollamaAI').get<number>('temperature', 0.3);
  }
