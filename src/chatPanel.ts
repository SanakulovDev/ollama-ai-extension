...
import { buildMultiFileContext, buildPrompt, getActiveFileContext, getSelectionContext } from './fileContext';
  ...
  private async _handleSend(userText: string, attachedPaths: string[] = []) {
    ...
    if (this._skill === 'code') {
      prompt = buildPrompt(
        ...
        attachedPaths.length > 0 ? buildMultiFileContext(this._dedupePaths(attachedPaths)) : ''
      );
    } else {
